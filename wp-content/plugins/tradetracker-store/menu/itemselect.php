...
<div id="TB_window1" style="left: auto;margin-left: auto;margin-right: auto; margin-top: 0;right: auto;top: 48px;visibility: visible;z-index:100051;width: <?php echo $adminwidth; ?>px;">
	<div id="ttstorebox">
		...
		<?php if(!isset($_GET['title'])&&!isset($_GET['description'])&&!isset($_GET['extra'])){ ?>
			...
			<input type="checkbox" name="extra" checked="checked" value="yes">extra fields
		<?php 
			$filterurl = '';
			} else { ?>
			<?php 
			...
			if(isset($_GET['extra']) && $_GET['extra']=='yes'){
				$extraurl = '&extra=yes';
			} else {
				$extraurl = '';
			}
				$filterurl = $titleurl."".$descriptionurl."".$extraurl;
			?>
			...
			<input type="checkbox" name="extra" <?php if($_GET['extra']=='yes'){ echo "checked=\"checked\""; }?> value="yes">extra fields
		<?php } ?>
...
if(isset($_GET['search']) && $_GET['search']!=""){
	$keyword = $_GET['search'];
	if($_GET['title']=='yes'){
		$searchtitle = "MATCH(name) AGAINST ('$keyword'  IN BOOLEAN MODE)";
	}
	if($_GET['description']=='yes'){
		if(isset($searchtitle)){
			$searchdescription = "or MATCH(description) AGAINST ('$keyword'  IN BOOLEAN MODE)";
		} else {
			$searchdescription = "MATCH(description) AGAINST ('$keyword'  IN BOOLEAN MODE)";
		}
	}
	if($_GET['extra']=='yes'){
		if(isset($searchtitle) || isset($searchdescription)){
			$searchextra = "or `extravalue` LIKE '%$keyword%'";
		} else {
			$searchextra = "or `extravalue` LIKE '%$keyword%'";
		}
	}
